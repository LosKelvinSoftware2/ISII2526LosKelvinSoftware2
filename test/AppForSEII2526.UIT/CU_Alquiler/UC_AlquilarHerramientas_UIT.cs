...
namespace AppForSEII2526.UIT.CU_Alquiler
    {
        public class CU_AlquilerHerramientas_UIT : UC_UIT
        {
            ...
            public void CU4_flujoNormal()
            {
                ...
                Assert.True(detailsAlquiler_PO.CheckDatos("Juan Pérez", "Calle Falsa 123", DateTime.Now.ToString("dd/MM/yyyy"), "900", DateTime.Now.AddDays(1).ToString("dd/MM/yyyy"), DateTime.Now.AddDays(7).ToString("dd/MM/yyyy")));
                Assert.True(detailsAlquiler_PO.CheckTablaHerramientas(expectedHerramientas));
            }
            [Fact]
            [Trait("LevelTesting", "Funcional Testing")]
            ...
            [Fact]
            [Trait("LevelTesting", "Funcional Testing")]
            public void CU4_AF0_AF0_AF2_Examen()
            {
                //Arrange
                Initial_step_opening_the_web_page();
                InitialStepsForAlquilarHerramientas();
                //Act
                selectHerramientasForAlquiler_PO.BuscarHerramientas("Lijadora", "");
                selectHerramientasForAlquiler_PO.AddHerramientaCarrito("Lijadora Orbital");
                selectHerramientasForAlquiler_PO.BuscarHerramientas("", "Acero");
                selectHerramientasForAlquiler_PO.AddHerramientaCarrito("Amoladora Angular");
                selectHerramientasForAlquiler_PO.SeguirConAlquiler();
                createAlquiler_PO.ModificarHerramientas();
                selectHerramientasForAlquiler_PO.RemoveAlquilerFromRentingCart("Lijadora Orbital");
                selectHerramientasForAlquiler_PO.SeguirConAlquiler();
                createAlquiler_PO.RellenarDatos("Juan", "Pérez", "Calle Falsa 123", "Tarjeta de crédito", "600123456", "PayPal", DateTime.Now.AddDays(1), DateTime.Now.AddDays(7));
                createAlquiler_PO.enviarFormulario();
                createAlquiler_PO.confirmarAlquiler();

                var expectedHerramientas = new List<string[]> { new string[] { "Amoladora Angular", "Acero", "130 €", "1" }, };
                //Assert
                Assert.True(detailsAlquiler_PO.CheckDatos("Juan Pérez", "Calle Falsa 123", DateTime.Now.ToString("dd/MM/yyyy"), "780", DateTime.Now.AddDays(1).ToString("dd/MM/yyyy"), DateTime.Now.AddDays(7).ToString("dd/MM/yyyy")));
                Assert.True(detailsAlquiler_PO.CheckTablaHerramientas(expectedHerramientas));
            }
        }
    }
